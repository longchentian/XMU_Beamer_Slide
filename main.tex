% !TeX document-id = {13e2f2d4-f224-4184-a23b-24063f20742d}
% !TeX encoding = UTF-8
...
% ----------------
\title[厦门大学虚拟偶像研究 | 化坤掌、苏珊六式、坤字诀、坤闪]{厦门大学虚拟偶像研究}
\subtitle{Beamer模板使用答辩} % subtitle 未设置页脚显示项, 请在 title 中设置.
\author[龙晨天, Linrong~Wu$^{\dagger}$]{龙晨天\inst{1,2} \and Linrong~Wu$^{3\dagger}$}
\institute{%
	\inst{1} 坤门
	~(\textit{KM@kun.com})
	\vspace*{-6pt} \and
	\inst{2} ~School of Aerospace Engineering, Xiamen University, Xiamen, China\\
	\inst{3} ~Business School, Sichuan University, Chengdu, China\\
...
\begin{frame}{关于本模板}%=0.85
	\begin{itemize}
		...
		\end{itemize}
		\item \textbf{修改者初衷}: \textbf{这模板设计的还不错，符合我对\LaTeX{}做PPT的想象}：
		\item 项目地址:
		\begin{itemize}
			\item 使用前请前往下列地址中查看原始模板版本!(有更新了看这个,跟着修改就行)
			\item \faGithub\enspace{\color{univblue}\url{https://github.com/FvNCCR228/SCU_Beamer_Slide-demo}}
			\item Gitee: \color{univblue}\url{https://gitee.com/NCCR/SCU_Beamer_Slide-demo}
		...
		\item 联系方式:
		\begin{itemize}
			\item 请联系原始制作者: lr.wu.interact@outlook.com
			\item 修改作者: 无名之辈--龙晨天
		\end{itemize}
	\end{itemize}
\end{frame}
...
\begin{frame}[fragile,allowframebreaks]{数学环境}
	...
	\begin{univproof}{}
		请读者自证.
	\end{univproof}
	\begin{univexample}{形翼门的规模}[HunyuanXytjmdgm]
