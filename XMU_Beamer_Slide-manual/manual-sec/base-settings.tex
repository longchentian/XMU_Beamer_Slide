...
\begin{frame}{Info.}
	...
	\begin{multicols}{2}
		\begin{itemize}
			...
			\item \myupdate{v1.3e}{2024/07/30}
		\end{itemize}
	\end{multicols}
...
\subsection{\pkg{beamethemeuniv} 宏包参数}
\begin{frame}{Info.}
	\textbf{本小节将介绍 \pkg{beamethemeuniv} 宏包参数.}
	\begin{multicols}{2}
		\begin{itemize}
...
\begin{frame}{\pkg{beamethemeuniv} 宏包参数键值对说明}{MathFont, ColorDisplay, CodeDisplay}
	\pkg{beamethemeuniv} 是基于\LaTeX{} beamer开发的四川大学beamer样式主题宏包, 可通过命令 \cmd{usetheme}\marg*{univ}调用.
...
\begin{frame}{\pkg{beamethemeuniv} 宏包参数键值对说明}{MintedStyle, BlockDisplay, Miniframes, LanguageMode}
	\vspace*{-1.6ex}
	\begin{columns}[T, onlytextwidth]% https://tex.stackexchange.com/questions/7442
		\begin{column}{.5\textwidth}
			\textbf{调用命令}(无需额外调用默认键值对):\\
...
\begin{frame}{\pkg{beamethemeuniv} 宏包参数键值对说明}{BIBMode, BIBStyle, ContentMuticols, Background}
	\begin{columns}[T, onlytextwidth]% https://tex.stackexchange.com/questions/7442
		\begin{column}{.5\textwidth}
			\textbf{调用命令}(无需额外调用默认键值对):\\
			\cmd{usetheme}\oarg*{\%\\
...
\subsection{文档信息填写}
\begin{frame}{Info.}
	\textbf{本小节将介绍 \pkg{univ} 主题样式中封面及页脚基本信息的填写.}
	\begin{multicols}{2}
		\begin{itemize}
			\item \myestablish{v1.1a}{2021/12/30}
			\item \myupdate{v1.3d}{2024/05/18}
...
\begin{frame}{封面及页脚信息}
  ...
  \begin{columns}[T, onlytextwidth]% https://tex.stackexchange.com/questions/7442
      ...
      \cmd{institute}\marg*{\%\\
        ...
        \hspace*{1em}\alert{\textbackslash\textbackslash}(\cmd{textit}\marg*{MaXJFake@univ.edu.cn})\\
      }
		\end{column}
...
% 			\scriptsize%
% 			{\color{univred}<text.f>}\\
% 			\quad 指该部分键入的文字在页脚中显示\\
% 			{\color{univred}<text.t>}\\
