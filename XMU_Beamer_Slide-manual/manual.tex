% !TeX document-id = {26a94b5f-b996-4943-a354-93debc224c6e}
% !TeX encoding = UTF-8
...
% 本文件为 UNIV_Beamer_Slide-demo 用户手册源文件.
% !使用前请阅读用户手册.
...
% --------UNIV Beamer 模板宏包--------
% ----------------
\usetheme[CodeDisplay=minted, Background=true]{univ}
...
\begin{frame}{关于本模板}%=0.85
	\begin{itemize}
		...
		\begin{itemize}
			...
			\item \faGithub\enspace{\color{univblue}\url{https://github.com/FvNCCR228/UNIV_Beamer_Slide-demo}}
...
\begin{frame}{字体}{字体强调}
  ...
  \textbf{调用命令}: \cmd{structure}\textcolor{univgrey}{<\Arg{overlay}>}\marg{text}\hfill\structure{Neighbor 老王}\\
  \textbf{调用环境}:\\
  \cmd{begin}\marg{structureenv}\textcolor{univgrey}{<\Arg{overlay}>}\\
  \hspace*{1em}\Arg{environment contents}\\
  \cmd{end}\marg{structureenv}\\[2ex]
  
  \structure{重点字体}\\
  
  \textbf{调用命令}: \cmd{alert}\textcolor{univgrey}{<\Arg{overlay}>}\marg{text}\hfill\alert{Neighbor 老王}\\
  \textbf{调用环境}:\\
  \cmd{begin}\marg{alertenv}\textcolor{univgrey}{<\Arg{overlay}>}\\
  \hspace*{1em}\Arg{environment contents}\\
...
\begin{frame}[fragile]{标点符号与颜色}
	\scriptsize
	% 标点符号部分源自武汉大学学位论文模板.
	\begin{univcode}[listing side text]{标点符号}*[BiaodianFh]{tex}
		建议使用半角标点符号, 后键入一个空格. (尤其是英文书写!)\\[1ex]
		双引号由两对左单引号、右单引号构成: ``  ''. 左单引号在键盘上ESC键下方.\\[1ex]
		建议使用实心的句号, 只要书写的是自然科学类文章.
	\end{univcode}
	\begin{univcode}[listing side text]{颜色}*[YanS]{tex}
		模板依VIS手册定义了以下颜色:\\
...
\begin{frame}[fragile]{单张图片的插入}
	\begingroup
	\scriptsize
	支持格式: {\color{univred}pdf, eps, png, jpg}. 建议使用矢量图片(svg建议下载~\href{https://inkscape.org/}{\color{univblue}Inkscape}~导出pdf)
	\endgroup
	\begin{minipage}{0.56\linewidth}
		\begin{univcode}[comment={%
				\scriptsize\vskip.5ex%
				{\color{univred}<position>}\quad 浮动体摆放的位置\\
				\quad \textbf{参数}: {\color{univblue}h}-此处, {\color{univblue}t}-顶部, {\color{univblue}b}-底部, {\color{univblue}p}-独立成页, \\
				{\color{univblue}!}-决定位置时忽略限制\\
				\quad \textbf{注}: 常用{\color{univblue}h}, {\color{univblue}htbp}, {\color{univblue}htbp!}. 参数顺序不作限制\\
				{\color{univred}<keys>}\quad 限制图片大小等\\
				\quad \textbf{参数}: {\color{univblue}width=?}-宽度, {\color{univblue}height=?}-高度, \\
				{\color{univblue}scale=?}-缩放, {\color{univblue}angle=?}-逆时针旋转角度\\
				\quad \textbf{注}: 宽高不建议同时使用, 以columnwidth设置宽度\\
				{\color{univred}<file>}\quad 文件名称(不要有空格)\\
				{\color{univred}<title>}\quad 图片标题\\
				{\color{univred}<label>}\quad 交叉引用标签\\
			},%
		...
		\begin{univcode}[righthand width=0.2\textwidth,%
...
\begin{frame}[allowframebreaks,fragile]{多图插入(子图, 并列小图)}
	...
	\begin{enumerate}
		...
		\begin{univcode}[comment={%
				\scriptsize\vskip1ex%
				{\color{univred}<width>}\quad 子图宽度\\
				{\color{univred}<code>}\quad 同上页figure环境\\
			},%
	...
	\end{enumerate}
	\begin{univcode}[listing side text,righthand width=.36\columnwidth]{子图示例}[ZituSl]{tex}
		\begin{figure}[h]
		...
		\end{figure}
	\end{univcode}
	\begin{univcode}[listing side text,righthand width=.36\columnwidth]{并列小图示例}[BinlieXtsl]{tex}
		\begin{figure}[h]
...
\begin{frame}{代码区块(含从文件读入)环境及命令定义}
	本模板定义了代码区块环境 \env{univcode} 与命令 \cmd{univcodeinput} 和 \cmd{univcodeinputnocounter}.\par\vspace{1ex}
	
	\textbf{调用环境(无星号环境)}:\\
	\cmd{begin}\marg*{univcode}\oarg{tcb options}\marg{title}\textcolor{univgrey}{\Arg{switch star}}\oarg{label suffix}\marg{code language}\oarg{code options}\textcolor{univgrey}{<\Arg{escapeinside}>}\\
	\hspace*{1em}\Arg{source code}\\
	\cmd{end}\marg*{univcode}\\[1ex]
	\textbf{调用环境(带星号环境)}:\\
	\cmd{begin}\marg*{univcode*}\oarg{tcb options}\marg{title}\textcolor{univgrey}{\Arg{switch star}}\oarg{label suffix}\marg{code language}\oarg{code options}\textcolor{univgrey}{<\Arg{escapeinside}>}\\
	\hspace*{1em}\Arg{source code}\\
	\cmd{end}\marg*{univcode*}\\[1ex]
	...
	\textbf{调用命令(区块显示序号)}:\\
	\cmd{univcodeinput}\oarg{tcb options}\marg{title}\textcolor{univgrey}{\Arg{switch star}}\oarg{label suffix}\marg{code language}\oarg{code options}\marg{filename}\textcolor{univgrey}{<\Arg{escapeinside}>}\\[1ex]
	\textbf{调用命令(区块不显示序号)}:\\
	\cmd{univcodeinputnocounter}\oarg{tcb options}\marg{title}\textcolor{univgrey}{\Arg{switch star}}\oarg{label suffix}\marg{code language}\oarg{code options}\marg{filename}\textcolor{univgrey}{<\Arg{escapeinside}>}
\end{frame}
...
\begin{frame}{定理区块环境信息}
	...
	\begin{table}[htbp!]
		...
		\end{tabular}
	\end{table}
	其中证明环境(univproof)结尾带有证毕符号(\cmd{qed}).$\textrightarrow$\qed
\end{frame}
	
\begin{frame}{定理区块环境命令}
	\textbf{调用环境(无星号环境)}(除univproof与univremark):\\
	\cmd{begin}\marg{env}\textcolor{univgrey}{<\Arg{overlay}>}\oarg{tcb options}\marg{title}\textcolor{univgrey}{\Arg{switch star}}\oarg{label suffix}\\
	\hspace*{1em}\Arg{environment contents}\\
	...
	\cmd{begin}\marg{env}\textcolor{univgrey}{<\Arg{overlay}>}\oarg{tcb options}\marg{title}\oarg{label suffix}\\
	\hspace*{1em}\Arg{environment contents}\\
	\cmd{end}\marg{env}\\[1ex]
...
\cprotEnv\begin{frame}
	...
	\begin{univcode}[comment={%
			\scriptsize%
			{\color{univred}<keys>}\quad frame环境选项\\
			~~常用选项: {\color{univblue}fragile}: 保护脆弱命令(如代码, 抄录环境需此项)\\
			~~{\color{univblue}allowframebreaks(=?)}: 允许内容过多时自动切帧(括号中省略即自动判断). 注: 使用\texttt{pagebreak}或\texttt{framebreak}命令可实现手动位置切帧换页, 但会在一些时候失效(不知道为什么, 得看源码)\\
			~~{\color{univblue}t}: 在页面顶部(默认居中)\\
			{\color{univred}<title>}\quad 标题\\
		},%
		...
		\begin{column}[t]{0.35\linewidth}
			...
			\begin{figure}[h]
				\begin{center}
					...
					\\厦门大学校徽及校名\\
					\vspace{1ex}
					\includegraphics[width=.4\columnwidth]{XMU-SoAE.png}
					\\厦门大学航空航天学院
				\end{center}
			\end{figure}
		\end{column}
...
\begin{frame}{Tcolorbox}
	...
	\begin{minipage}[c]{0.725\columnwidth}
		\begin{univtcbrb}{锦程梦研}
			锦程梦研主题: 锦秀红与宝石蓝为渐变底色.
		\end{univtcbrb}
		\begin{univtcbyg}{浮莲落杏}
			浮莲落杏主题: 荷叶绿与银杏黄为渐变底色.
		\end{univtcbyg}
	\end{minipage}~~~~
	...
	\end{minipage}
	\newline\par
	\color{univgreen}此处仅展示示例, 具体操作请查看宏包手册 - ``tcolorbox.pdf''.
\end{frame}
...
%		\scalebox{20}{}
	...
	\end{minipage}
	\par
	\color{univgreen}此处仅展示示例, 具体操作请查看宏包手册 - ``pgfmanual.pdf''.
\end{frame}
...
\structure{	参考文献空白请按F8启动bibtex进行编译)。(texstudio)}
...
\begin{frame}
	...
	\end{columns}
	\centering\vskip4ex
	UNIV Beamer Slide-demo, Rev 1.3e
\end{frame}
